...
void ColorLevelsDialog::setupUI()
{
    ...
    // Reset and picker buttons layout
    ...
    inputGammaSlider->setRange(10, 1000); // Gamma from 0.1 to 10.0
    ...
    inputGammaSlider->setValue(100); // Gamma 1.0
...
void ColorLevelsDialog::updateLevels()
{
    ...
    blackPoint += fineAdjustBlackSlider->value();
    whitePoint += fineAdjustWhiteSlider->value();

    double gamma = inputGammaSlider->value() / 100.0; // Gamma ranges from 0.1 to 10.0

    if (gamma <= 0) gamma = 0.01; // Avoid division by zero

    // Reset the working image before applying the new levels
    ...
    // Apply the levels adjustment to each pixel
    for(int y = 0; y < workingImage.height(); ++y) {
        QRgb *scanLine = reinterpret_cast<QRgb*>(workingImage.scanLine(y));
        for(int x = 0; x < workingImage.width(); ++x) {
